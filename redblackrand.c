...
void main()
{
	...
	FILE *pipe;
	NODE root=TNULL;
	outputFile = fopen ("rbr.dot", "w");
    	fclose (outputFile);
	int ch,val,i;
	printf("Enter the number of keys in tree\n");
			scanf("%d",&ch);
	srand(time(NULL));
	for(i=0;i<ch;i++)
	{
		...
		outputFile = fopen ("rbr.dot", "a");
		if (outputFile != NULL) 
			dotDump (root,  outputFile);
		fclose (outputFile);
	}
	pipe=popen("dot -Tps rbr.dot -o rbr.ps","w");
	pclose(pipe);	
	pipe=popen("evince rbr.ps","r"); 
	pclose(pipe);
}
NODE getNode()
{
